...
'''def aggregate_child_nodes(aggregated_node, child_node, path_to_node_map):
    ...
    if corresponding_child:
        ...
        for grandchild in child_node.children:
            print("Aggregating grand child node", grandchild.path)
            aggregate_child_nodes(corresponding_child, grandchild, path_to_node_map)
    else:
        # If no corresponding child exists, add the new child to the aggregated node
        print("Added new child instead", child_node.path)
        aggregated_node.add_child(deepcopy(child_node))
        path_to_node_map[tuple(child_node.path)] = child_node  # Update the map with new child

def aggregate_root_results(root_nodes):
    aggregated_root = deepcopy(root_nodes[0])  # Create a deep copy of the first root
    print("Deep copied root")
    path_to_node_map = {tuple(child.path): child for child in aggregated_root.children}  # Map paths to children
    print("Created path to node map")
    ...
    for i in range(1, len(root_nodes)):
        for child in root_nodes[i].children:
            ...
            print("Aggregating child node", child.path)
            aggregate_child_nodes(aggregated_root, child, path_to_node_map)
...
def mcts_run_parallel_root(graph: OrienteeringGraph, start_node_index=0, num_simulations=1000000, num_threads=4):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
        futures = [executor.submit(run_single_mcts, graph, root, num_simulations) for root in root_nodes]
        parallel_roots = [f.result() for f in futures]

    # Aggregate the results of all parallel root nodes
    aggregated_root = aggregate_root_results(parallel_roots, graph)

    # Select the best child of the aggregated root based on visits or value
